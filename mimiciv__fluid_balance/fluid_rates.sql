...
SELECT 
  ...
  starttime,
  endtime,
  COALESCE(blood_products,0) AS blood_products,
  COALESCE(bolus,0) AS bolus,
  COALESCE(colloids,0) AS colloids,
  COALESCE(crystalloids,0) AS crystalloids,
  COALESCE(drips,0) AS drips,
  COALESCE(enteral_nutrition,0) AS enteral_nutrition,
  COALESCE(oral_gastric_intake,0) AS oral_gastric_intake,
  COALESCE(parenteral_nutrition,0) AS parenteral_nutrition,
  COALESCE(supplements,0) AS supplements,
  COALESCE(blood_products,0)+COALESCE(bolus,0)+COALESCE(colloids,0)+COALESCE(crystalloids,0)+COALESCE(drips,0)+COALESCE(enteral_nutrition,0)+COALESCE(oral_gastric_intake,0)+COALESCE(parenteral_nutrition,0)+COALESCE(supplements,0) AS total_rate
  FROM rates
